                                     ...
                                     int numCRows, int numCColumns)
{
    ...
    // defined above : TSZ_COLS = 16,   TSZ_ROWS = 64,    TSZ_STRIPS = 4
	...
	size_t btile_col = tx % TSZ_COLS;
	size_t btile_row = tx / TSZ_COLS;

    // map thread index to C's row and columns (as 1 thread
    // will produce TSZ_COLS entries)
    size_t thread_blocks_per_row = numBColumns / TSZ_COLS;
    size_t output_row = blockIdx.x / thread_blocks_per_row * blockDim.x + threadIdx.x;
    size_t output_col = blockIdx.x % thread_blocks_per_row * TSZ_COLS;

    for (size_t k = 0; k < numBRows; k += TSZ_STRIPS) {
		// read B data into SMEM
        if(k + btile_row < numBRows && output_col + btile_col < numBColumns){
		    bdata_smem[btile_row][btile_col] = B[(k + btile_row)*numBColumns + (output_col + btile_col)];
        }else{
            bdata_smem[btile_row][btile_col] = 0;
        }
		...
		for (int i = 0; i < TSZ_STRIPS; i++) {
            if(output_row < numARows && k + i < numAColumns){
		        adata_regs[i] = A[output_row*numAColumns + (k + i)];
            }
		}
	...
	for (int i = 0; i < TSZ_COLS; i++) {
        if(output_row < numCRows && output_col + i < numCColumns){
		    C[output_row*numCColumns + (output_col + i)] = result_regs[i];
        }
    }
...
// ------------------------------------------------------------------------
    ...
    //     // in this implementation i = tileId * TILE_WIDTH
    //     if (row < numARows && i + tx < numAColumns) {
    //         tileA[ty][tx] = A[(size_t) row * numAColumns + i + tx];
    //     } else {
    //         tileA[ty][tx] = 0;
    //     }
    //     if (col < numBColumns && i + ty < numBRows) {
    //         tileB[ty][tx] = B[((size_t) i + ty) * numBColumns + col];
    //     } else {
    //         tileB[ty][tx] = 0;
    //     }
    //     __syncthreads();
...
//-----------------------------------------------------------------------------------------
    ...
    // in this implementtaion SIZE = numAColumsn or numBColumns or numARpws, it depends
                                                ...
                                                const int Height, const int Width, const int K)
{
    ...
    dim3 gridDim2((((Map_out * Width_unrolled) + TSZ_ROWS -1) / TSZ_ROWS), 1, 1);
    dim3 blockDim2(TSZ_ROWS, 1, 1);
