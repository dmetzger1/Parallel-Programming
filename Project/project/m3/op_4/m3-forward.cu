...
__host__ void GPUInterface::conv_forward_gpu(float *device_output, const float *device_input, const float *device_mask, const int Batch, const int Map_out, const int Channel, const int Height, const int Width, const int K)
{
    ...
    // Use Cublas for Matrix Multiplication, transpose device_mask and unrolled_matrix
    cublasSgemm(cublas_handle, CUBLAS_OP_N, CUBLAS_OP_N,  
                Width_unrolled, Map_out, Height_unrolled,
                &alpha, unrolled_matrix, Width_unrolled,  
                device_mask, Height_unrolled,  
                &beta, matmul_output, Width_unrolled);  // Output stored in column-major order by default
