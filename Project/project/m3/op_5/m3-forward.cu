...
void float_to_half(const float* h_src, __half* h_dst, int size) {
    for (int i = 0; i < size; ++i) {
        h_dst[i] = __float2half(h_src[i]);
    }
}

// Convert FP16 to FP32
void half_to_float(const __half* h_src, float* h_dst, int size) {
    for (int i = 0; i < size; ++i) {
        h_dst[i] = __half2float(h_src[i]);
    }
}
                                                    ...
                                                    const int Width, const int K)
{
    ...
    //     exit(-1);
    // }
    __half *host_input_half;
    __half *host_mask_half;  

    int Height_out = Height - K + 1;
    int Width_out = Width - K + 1;

    int input_size = Batch * Channel * Width * Height * sizeof(__half);
    int output_size = Batch * Map_out * Height_out * Width_out * sizeof(float);
    int mask_size = Map_out * Channel * K * K * sizeof(__half);

    cudaMalloc((void**) device_input_ptr, input_size);
    cudaMalloc((void**) device_output_ptr, output_size);
    cudaMalloc((void**) device_mask_ptr, mask_size);

    host_input_half = (half*)malloc(input_size);
    host_mask_half = (half*)malloc(mask_size);

    for (int i = 0; i< Batch*Channel*Height*Width; i++){
        host_input_half[i] = __half(host_input[i]);
    }
    for (int i = 0; i< Map_out*Channel * K * K; i++){
        host_mask_half[i] = __half(host_mask[i]);
    }

    cudaMemcpy(*device_input_ptr, host_input_half, input_size, cudaMemcpyHostToDevice);
    cudaMemcpy(*device_mask_ptr, host_mask_half, mask_size, cudaMemcpyHostToDevice);
                                            ...
                                            const int Height, const int Width, const int K)
{
    ...
    matrix_unrolling_kernel<<<gridDim1, blockDim1>>>((half*)device_input, unrolled_matrix, Batch, Channel, Height, Width, K);
    ...
    matrixMultiplyShared<<<gridDim2, blockDim2>>>((half*)device_mask, unrolled_matrix, matmul_output,
